...
const DataWrapper: React.FC<{
  ...
  children: any
  props?: any
}> = ({ nowrapper, children, ...props }) => {
  return (
    <MainWrapper nowrapper={nowrapper}>
      <DataChildComponent data={props}>{children}</DataChildComponent>
    </MainWrapper>
  )
}

const ChildComponent: React.FC<{ children: any; data: any }> = ({
  children,
  data
}) => {
...
const MainWrapper: React.FC<{
  ...
  return (
    <>
      {nowrapper != null ? <>{children}</> : <Dashboard>{children}</Dashboard>}
    </>
  )
}
